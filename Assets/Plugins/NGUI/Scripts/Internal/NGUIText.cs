...
static public class NGUIText
{
	...
	static public BMSymbol GetSymbol (string text, int index, int textLength)
	{
        if (bitmapFont != null)
        {
            BMSymbol temp = bitmapFont.MatchSymbol(text, index, textLength);
            temp.text = text[index].ToString();
            return temp;
        }
		return null;
	}
    ...
    static public void Align(List<Vector3> verts, int indexOffset, float printedWidth, int elements = 4, string text = "", int force = 0)
	    {
		    switch (alignment)
		    {
			    ...
			    case Alignment.Justified:
			    {
				    ...
				    if ((printedWidth < rectWidth * 0.65f && force < 1) || force == 0) return;
                    ...
                    List<int> spacesIndexes = new List<int>(0);
                    int currentId = 0;
                    int spacesCount = 0;
                    ...
                    float spacesForSpace = (rectWidth - printedWidth) / spacesCount;

                    float createdSpace = 0;

                    for (int i = indexOffset + elements, charIndex = 1, imax = verts.Count; i < imax; ++charIndex)
                    {
                        scale = 1;
                        if (spacesIndexes.Contains(charIndex))
                            createdSpace += spacesForSpace;

                        float x0 = verts[i].x + createdSpace;
                        float x1 = verts[i + elements / 2].x + createdSpace;
					    float w = x1 - x0;
	...
	static public void Print (string text, List<Vector3> verts, List<Vector2> uvs, List<Color> cols)
	{
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (ch == '\n')
			{
				...
				{
                    Align(verts, indexOffset, x - finalSpacingX, 4 , lineText, i == textLength - 1 ? 0 : 1);
			...
			if (symbol != null)
			{
				...
				if (x + w > maxWidth)
				{
					...
					{
                        Align(verts, indexOffset, x - finalSpacingX, 4, lineText, 1);
                        Debug.Log(lineText);
                        lineText = "";
                        indexOffset = verts.Count;
					}
			...
			{
				GlyphInfo glyph = GetGlyph(ch, prev, scale);
				if (glyph == null) continue;
                glyph.text = text[i].ToString();
                lineText += text[i];
                prev = ch;
		...
		if (alignment != Alignment.Left && indexOffset < verts.Count)
		{
			Align(verts, indexOffset, x - finalSpacingX, 4, lineText);
            //Debug.Log(lineText);
            lineText = "";
            indexOffset = verts.Count;
		}
